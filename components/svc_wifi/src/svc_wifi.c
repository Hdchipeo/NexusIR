...
esp_err_t svc_wifi_start(void *pop_info) {
    ...
    (CONFIG_LAMP_PLATFORM_IOS && CONFIG_IOS_WIFI_PROV_AP_ENABLE)
  // SoftAP provisioning (User selected SoftAP OR iOS with SoftAP enabled)
  ...
  if (svc_wifi_prov_creds_exist()) {
    ...
    if (svc_wifi_prov_creds_load(ssid, password) == ESP_OK) {
...
esp_err_t svc_wifi_update_credentials(const char *ssid, const char *password) {
  wifi_config_t wifi_config = {0};
  strlcpy((char *)wifi_config.sta.ssid, ssid, sizeof(wifi_config.sta.ssid));
  strlcpy((char *)wifi_config.sta.password, password,
          sizeof(wifi_config.sta.password));
...
esp_err_t svc_wifi_get_scan_results(wifi_ap_record_t **ap_list,
                                    uint16_t *count) {

...
esp_err_t drv_aht20_init(void) {
  ...
  vTaskDelay(pdMS_TO_TICKS(50)); // Allow sensor stabilization
  ...
  vTaskDelay(pdMS_TO_TICKS(100)); // Give sensor time to power up
  return ESP_OK;
}
...
esp_err_t aht20_sensor_read(float *temp, float *hum) {
  ...
  if (err == ESP_ERR_INVALID_STATE || err == ESP_FAIL) {
    ESP_LOGW(TAG, "AHT20 in invalid state, attempting re-init...");
    drv_aht20_init(); // Shallow re-init check
    vTaskDelay(pdMS_TO_TICKS(100));
    err = aht20_read_temperature_humidity(s_aht20, &t_raw, temp, &h_raw, hum);
  }
...
esp_err_t drv_aht20_init(void) { return ESP_OK; }
esp_err_t aht20_sensor_read(float *temp, float *hum) {

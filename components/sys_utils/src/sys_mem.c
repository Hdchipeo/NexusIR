...
static void sys_mem_task(void *pvParameters) {
  while (1) {
...
esp_err_t sys_mem_init(void) {
  ...
  xTaskCreate(sys_mem_task, "sys_mem_task", 4096, NULL, 5, NULL);
  return ESP_OK;
}
...
bool sys_mem_is_safe(size_t size, bool use_psram) {
  size_t free_mem =
      use_psram ? sys_mem_get_free_psram() : sys_mem_get_free_internal();
  size_t threshold =

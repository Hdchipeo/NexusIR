...
#if CONFIG_LAMP_PLATFORM_IOS && CONFIG_IOS_WIFI_PROV_AP_ENABLE
...
static const char *TAG = "svc_wifi_prov";
...
esp_err_t svc_wifi_prov_creds_load(char *ssid, char *password) {
  if (!ssid || !password) {
...
esp_err_t svc_wifi_prov_creds_clear(void) {
  nvs_handle_t handle;
  esp_err_t err = nvs_open(NVS_NAMESPACE, NVS_READWRITE, &handle);
  if (err != ESP_OK) {
    return err;
...
static esp_err_t connect_handler(httpd_req_t *req) {
  ...
  esp_err_t err = svc_wifi_prov_creds_save(ssid, password);
  cJSON_Delete(json);
...
// iOS captive portal detection - needs HTTP 200 with body != "Success"
// Serve the actual portal HTML directly to trigger the popup
static esp_err_t captive_portal_ios_handler(httpd_req_t *req) {
  ESP_LOGI("WiFi_Portal", "iOS captive portal probe detected: %s", req->uri);

  // Serve the actual portal page directly (same as index_handler)
  // This triggers iOS to show the captive portal dialog
  const size_t html_len = index_html_end - index_html_start;
...
// Android captive portal detection - expects HTTP 204, we return 302 to
// trigger popup
static esp_err_t captive_portal_android_handler(httpd_req_t *req) {
  ESP_LOGI("WiFi_Portal", "Android captive portal probe detected: %s",
           req->uri);
...
// Generic redirect handler for other requests
...
static httpd_handle_t start_webserver(void) {
  ...
  config.max_open_sockets = 4; // LWIP max: 7, HTTP internal: 3, available: 4
  ...
  // iOS Captive Portal Probes - use iOS handler (HTTP 200 with non-Success
  // body)
  const char *ios_probes[] = {
      "/hotspot-detect.html",
      "/library/test/success.html",
      "/success.txt",
  };
  for (int i = 0; i < sizeof(ios_probes) / sizeof(ios_probes[0]); i++) {
    httpd_uri_t probe_uri = {.uri = ios_probes[i],
                             .method = HTTP_GET,
                             .handler = captive_portal_ios_handler};
    httpd_register_uri_handler(server, &probe_uri);
  }

  // Android/Google Captive Portal Probes - use Android handler (302 redirect)
  const char *android_probes[] = {
      "/generate_204",
      "/gen_204",
      "/mobile/status.php",
  ...
  for (int i = 0; i < sizeof(android_probes) / sizeof(android_probes[0]); i++) {
    httpd_uri_t probe_uri = {.uri = android_probes[i],
                             .method = HTTP_GET,
                             .handler = captive_portal_android_handler};
    httpd_register_uri_handler(server, &probe_uri);
  }

  // Windows/Other probes - generic redirect
  const char *other_probes[] = {
      "/ncsi.txt",
      "/fwlink/*",
      "/portal.html",
  };
  for (int i = 0; i < sizeof(other_probes) / sizeof(other_probes[0]); i++) {
    httpd_uri_t probe_uri = {.uri = other_probes[i],
                             .method = HTTP_GET,
...
esp_err_t svc_wifi_prov_init(void) {
...
bool svc_wifi_prov_is_provisioned(void) { return provisioned_flag; }

#else

// Stub implementations when iOS provisioning is disabled
esp_err_t svc_wifi_prov_init(void) { return ESP_OK; }
esp_err_t svc_wifi_prov_start(void) { return ESP_OK; }
void svc_wifi_prov_stop(void) {}
bool svc_wifi_prov_is_provisioned(void) { return false; }
bool svc_wifi_prov_creds_exist(void) { return false; }
esp_err_t svc_wifi_prov_creds_load(char *ssid, char *password) {
...
esp_err_t svc_wifi_prov_creds_clear(void) { return ESP_ERR_NOT_SUPPORTED; }

#endif // CONFIG_LAMP_PLATFORM_IOS && CONFIG_IOS_WIFI_PROV_AP_ENABLE

...
async function scanNodes() {
    document.getElementById('modal-scan-nodes').classList.add('active');
    await fetchNodes();
    await startScanNodes();
}

async function startScanNodes() {
    const container = document.getElementById('scan-results-container');
    if (!container) return;
    
    container.innerHTML = `
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:30px 10px; color:var(--text-secondary); gap:10px;">
            <div class="spin" style="width: 24px; height: 24px; border: 3px solid var(--accent); border-top-color: transparent; border-radius: 50%;"></div>
            <span style="font-size:14px;">Đang quét thiết bị xung quanh...</span>
        </div>
    `;
    ...
    try {
        ...
        lastScannedDevices = await res.json();
        renderScanResults();
    } catch (e) {
        container.innerHTML = `
            <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:30px 10px; color:var(--danger);">
                <span style="font-size:14px;">Quét thất bại hoặc mất kết nối.</span>
            </div>
        `;
    }
}
...
function renderScanResults() {
    ...
    if (lastScannedDevices.length === 0) {
        container.innerHTML = `
            <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:30px 10px; color:var(--text-secondary);">
                <span style="font-size:14px;">Không tìm thấy thiết bị nào.</span>
            </div>
        `;
        return;
    }

    lastScannedDevices.forEach(dev => {
        const isAlreadyPeer = state.nodes.some(mac => mac.toLowerCase() === dev.mac.toLowerCase());
        const item = document.createElement('div');
        item.className = 'list-item';
        item.style.cssText = 'display:flex; justify-content:space-between; align-items:center; padding: 10px 12px; background: rgba(255,255,255,0.05); border-radius: 10px; margin-bottom: 8px;';
        
        const infoDiv = document.createElement('div');
        infoDiv.style.cssText = 'display:flex; flex-direction:column; gap:2px;';
        infoDiv.innerHTML = `
            <span style="font-weight:600; color:var(--text-primary); font-size:14px;">${dev.name || "Không rõ tên"}</span>
            <span style="font-size:12px; color:var(--text-secondary);">${dev.mac}</span>
        `;
        
        const btnDiv = document.createElement('div');
        if (isAlreadyPeer) {
            btnDiv.innerHTML = `<span style="font-size:12px; color:var(--success); font-weight:500;">Đã kết nối</span>`;
        } else {
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'btn';
            btn.style.cssText = 'padding: 6px 12px; font-size: 12px; background:var(--accent); color:white; border-radius: 8px;';
            btn.innerText = 'Thêm';
            btn.onclick = () => addScannedNode(dev.mac);
...
async function addScannedNode(mac) {
    try {
        const res = await fetch('/api/nodes', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },

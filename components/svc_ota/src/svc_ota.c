...
static void auto_update_task(void *pvParameter) {
  ...
  while (1) {
    ...
    if (svc_ota_check_version(version_buffer, sizeof(version_buffer)) ==
        ESP_OK) {
...
const char *svc_ota_get_cached_version(void) { return g_remote_version; }

bool svc_ota_is_update_available(void) { return g_update_available; }
